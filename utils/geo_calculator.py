...
class GeoCalculator:
    ...
    def fix_corrupted_coordinates(lat: float, lon: float) -> Tuple[float, float]:
        """🔧 CORRECCIÓN:  coordenadas corruptas detectadas en CSV"""
        try:
    ...
    def calculate_travel_time(distance_km: float,
        ...
        # Velocidades dinámicas por distancia
        if distance_km <= 20:
            # Urbano/local
        ...
        adjusted_speed = max(adjusted_speed, 25.0)
